  ...
  const handleSaveItem = async () => {
    ...
    try {
      if (editItem) {
        ...
        await supabase.from('venue_catalogue_items').insert({ listing_id: listing.id, title: form.title.trim(), description: form.description.trim() || null, price, currency: 'ZAR', sort_order: nextSort, is_active: form.is_active });
      }
      closeForm();
      await load();
