...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const subject = `Welcome to Funxons - Complete Your Vendor Application`;
    ...
    const htmlContent = `
      ...
      <head>
        ...
        <title>Welcome to Funxons</title>
      </head>
      <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="background: linear-gradient(135deg, #2D5A4C 0%, #4A7C6F 100%); padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
          <h1 style="color: white; margin: 0; font-size: 28px;">Welcome to Funxons!</h1>
        </div>
        
        <div style="background: #ffffff; padding: 30px; border: 1px solid #e0e0e0; border-top: none; border-radius: 0 0 10px 10px;">
          <p style="font-size: 18px; margin-bottom: 20px;">Hi ${fullName},</p>
          
          <p>Congratulations on choosing the <strong>${tierName}</strong> plan! You're now one step closer to becoming a Funxons vendor.</p>
          ...
          <p style="font-size: 14px; color: #666; margin-top: 5px;">The Funxons Team</p>
        </div>
      </body>
      </html>
    `;
...
Congratulations on choosing the ${tierName} plan! You're now one step closer to becoming a Funxons vendor.
