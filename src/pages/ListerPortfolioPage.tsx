    ...
    return (
      <div className="fx-container py-12">
        <div className="mx-auto max-w-4xl">
          <div className="h-[300px] animate-pulse rounded-xl bg-brand-pink" />
          <div className="mt-6 h-8 w-1/2 animate-pulse rounded bg-brand-pink" />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="fx-container py-6 md:py-10">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="relative overflow-hidden rounded-xl bg-brand-pink">
          {lister.image_url ? (
            <img src={lister.image_url} alt={lister.name} className="h-[250px] w-full object-cover md:h-[350px]" />
          ) : (
            <div className="flex h-[250px] w-full items-center justify-center bg-brand-pink md:h-[350px]">
        ...
        <div className="mt-6 grid gap-6 md:grid-cols-3">
          <div className="md:col-span-2">
            <div className="flex items-start justify-between">
              ...
              {lister.rating && (
                <div className="flex items-center gap-1 rounded-lg bg-primary-muted px-3 py-1.5">
                  <Star className="h-4 w-4 fill-current text-secondary" />
                  <span className="text-sm font-semibold text-primary">{lister.rating}</span>
                  {lister.review_count && <span className="text-xs text-primary">({lister.review_count})</span>}
                </div>
              )}
            </div>

            {lister.description && (
              <div className="mt-4 rounded-lg bg-brand-pink p-4">
          ...
          <div className="space-y-4">
            <div className="rounded-xl bg-white p-5 shadow-sm border border-border-subtle">
              ...
              <div className="mt-3 space-y-2">
                ...
                {lister.email && <a href={`mailto:${lister.email}`} className="fx-btn-ghost w-full border border-border-subtle"><Mail className="mr-2 h-4 w-4" /> Email</a>}
                {lister.website && <a href={lister.website} target="_blank" rel="noopener noreferrer" className="fx-btn-ghost w-full border border-border-subtle"><Globe className="mr-2 h-4 w-4" /> Website</a>}
