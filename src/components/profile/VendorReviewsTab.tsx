...
const VendorReviewsTab = React.memo(function VendorReviewsTab({
  reviews,
  reviewsLoading,
  reviewsError,
  ratingSummaryValue,
    ...
    <View>
      ...
      {user?.id ? (
        <View style={{ marginBottom: spacing.lg, padding: spacing.lg, borderRadius: radii.lg, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.borderSubtle }}>
          <Text style={{ ...typography.titleMedium, color: colors.textPrimary, marginBottom: spacing.sm }}>Leave a review</Text>
          <Text style={{ ...typography.body, color: colors.textSecondary, marginBottom: spacing.md }}>Share your experience with this vendor. Reviews help other users make informed decisions.</Text>
          <PrimaryButton title="Leave a review" onPress={() => navigation.navigate('CreateReview', { type: 'vendor', targetId: vendorId, targetName: name })} />
        </View>
      ) : (
        <View style={{ marginBottom: spacing.lg, padding: spacing.lg, borderRadius: radii.lg, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.borderSubtle }}>
      ...
      {reviewsLoading ? (
        ...
        <View style={{ marginBottom: spacing.lg, padding: spacing.lg, borderRadius: radii.lg, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.borderSubtle }}>
          <Text style={{ ...typography.titleMedium, color: colors.textPrimary, marginBottom: spacing.sm }}>Failed to load reviews</Text>
        ...
        <View style={{ gap: spacing.md, marginBottom: spacing.lg }}>
          {reviews.map((review) => (
            <View key={review.id} style={{ padding: spacing.lg, borderRadius: radii.lg, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.borderSubtle }}>
              ...
              {review.title ? <Text style={{ ...typography.titleMedium, color: colors.textPrimary, marginTop: spacing.sm }}>{review.title}</Text> : null}
              {review.review_text ? (
