...
export default function UpdatePortfolioScreen() {
    ...
    const loadVendor = useCallback(async () => {
        ...
        try {
            ...
            // Try to find vendor by user_id or by email match
            const { data: vendorData, error } = await supabase
                .from('vendors')
                .select('id, name, description, location, price_range, email, whatsapp_number, website_url, instagram_url, subscription_tier, service_options, amenities, vendor_tags')
    ...
    const renderField = (label: string, key: keyof typeof form, options?: { multiline?: boolean; placeholder?: string; keyboardType?: any }) => (
                ...
                {!vendor ? (
                    <View style={{ paddingHorizontal: spacing.lg, alignItems: 'center', paddingTop: spacing.xl }}>
                        ...
                        <Text style={{ ...typography.body, color: colors.textMuted, marginTop: spacing.sm, textAlign: 'center' }}>
                            You haven't created a portfolio yet. Create one to start listing your services.
                        </Text>
                        <TouchableOpacity
                            onPress={() => navigation.goBack()}
                            style={{
                            ...
                            <View style={{
                                ...
                                {vendor.service_options && vendor.service_options.length > 0 && (
                                    <View style={{ marginBottom: spacing.sm }}>
                                        <Text style={{ ...typography.caption, color: colors.textMuted, marginBottom: spacing.xs }}>Service Options</Text>
                                        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: spacing.xs }}>
                                            {vendor.service_options.map((opt, i) => (
                                                <View key={i} style={{ paddingHorizontal: spacing.sm, paddingVertical: spacing.xs, borderRadius: radii.full, backgroundColor: colors.surfaceMuted, borderWidth: 1, borderColor: colors.borderSubtle }}>
                                ...
                                {vendor.vendor_tags && vendor.vendor_tags.length > 0 && (
                                    <View>
                                        <Text style={{ ...typography.caption, color: colors.textMuted, marginBottom: spacing.xs }}>Tags</Text>
                                        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: spacing.xs }}>
                                            {vendor.vendor_tags.map((tag, i) => (
                                                <View key={i} style={{ paddingHorizontal: spacing.sm, paddingVertical: spacing.xs, borderRadius: radii.full, backgroundColor: '#E0F2F7', borderWidth: 1, borderColor: colors.primaryTeal }}>
